...
test_r001_sync_single_dist() {
  ...
  local src_skill_count
  src_skill_count="$(find "$REPO_DIR/skills" -name "SKILL.md" -not -path "*/_shared/*" | wc -l)"
  assert_eq "R-001: correctless/ skill count matches source" "$src_skill_count" "$skill_count"

  # R-001: verify each skill exists in correctless/
  for skill in csetup cspec cmodel creview creview-spec ctdd cverify caudit cupdate-arch cdocs cpostmortem cdevadv credteam crefactor cpr-review ccontribute cmaintain cstatus csummary cmetrics cdebug chelp cwtf cquick crelease cexplain cauto carchitect cmodelupgrade cdashboard ctriage; do
