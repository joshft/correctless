...
check_inv027() {
  ...
  # Body moved to the abstractions fragment (index+body-out fragmentation); the
  # heading stays in root (checked by INV-027(e) above).
  if grep -A 30 '^### ABS-027:' docs/architecture/abstractions.md | grep -qE 'harness[ -]fingerprint|harness-fingerprint\.json'; then
...
check_path_discovery_guard() {
  ...
  local EXCLUDED_FROM_DISCOVERY="crelease cupdate-arch carchitect csetup chelp cstatus cquick cexplain cdebug crefactor ccontribute cmaintain cpr-review credteam caudit cdevadv cauto cwtf cmetrics cspec cdashboard ctriage cprune cchores"
