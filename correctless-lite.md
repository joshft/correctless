...
> **Note:** This is the original design specification. The implementation has evolved — 10 skills were added (/csetup, /cstatus, /csummary, /cmetrics, /cdebug, /cpr-review, /crefactor, /ccontribute, /cmaintain, /chelp), the security checklist was added to /creview, and the research agent was added to /cspec. Lite now has 15 skills; Full has 22. See the [README](README.md) and the actual [skill files](skills/) for the current implementation.
...
Same paradigm as Correctless, stripped to the essentials. Fifteen skills. Lightweight version of the full twenty-two-skill suite. One reviewer instead of an agent team. No Alloy, no STRIDE, no convergence loops, no external model integration.
