...
- **build-dashboard.sh** — HTML dashboard UI generator with metrics + artifact browser views.
...
HTML dashboard UI (`/cdashboard`) for longitudinal quality visualization and artifact browsing. Generated by `scripts/build-dashboard.sh` as `.correctless/dashboard/index.html`. Two views:
...
- **Artifact Browser** — sidebar navigation for specs, verifications, review findings, research briefs, architecture docs, QA findings, audit history

Uses marked.js + DOMPurify (CDN, SRI-pinned) for safe markdown rendering. Supports light/dark mode. Graceful degradation when CDN unavailable.
