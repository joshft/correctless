...
The writer script handles branch_slug derivation, directory creation, and JSONL append internally. Routing all writes through this single script is a sole-writer (who-writes) convention; SFG guards only the Edit/Write tool-path, so a direct Bash redirect to the JSONL is no longer blocked (accepted residual, AP-040). Unlike ABS-029/audit-record.sh, the autonomous-decisions JSONL (ABS-030) has NO `cmd_*` content gate behind it — the sole-writer convention is the only structural leg, and the residual is a Tier-2 accepted non-goal. Each entry has fields: `skill` (string), `decision_id` (AD-xxx or AD-UNLISTED-N), `default_applied` (string), `rationale` (string), `timestamp` (ISO 8601), `escalation_deferred` (boolean, default false), `original_escalation_reason` (string, null unless escalation_deferred is true).
