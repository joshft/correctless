...
**Branch guard**: Check the current branch with `git rev-parse --abbrev-ref HEAD`. If on `main` or `master`, stop immediately and tell the user: "You're on the main branch. Create a feature branch first: `git checkout -b feature/my-feature`, then re-run /cspec." Do not proceed with the brainstorm — context gathered here will be lost when the user switches branches.

**First-run check**: If `.correctless/ARCHITECTURE.md` contains `{PROJECT_NAME}` or `{PLACEHOLDER}` markers, or if `.correctless/config/workflow-config.json` does not exist, tell the user: "Correctless isn't fully set up yet." Check if `.correctless/scripts/lib.sh` exists — if it does not, the workflow infrastructure is missing and the quick-scan fallback will crash at `workflow-advance.sh init`. In that case, tell the user: "Run `/csetup` first — the workflow requires infrastructure files that only setup installs." Do NOT offer the quick-scan. If `.correctless/scripts/lib.sh` exists (infrastructure is present but docs are missing), offer: "I can do a quick scan of your codebase right now to populate .correctless/ARCHITECTURE.md and .correctless/AGENT_CONTEXT.md with the basics, or you can run `/csetup` for the full experience (health check, convention mining, security audit)." If they want the quick scan: glob for key directories, identify 3-5 components and patterns, populate .correctless/ARCHITECTURE.md with real entries, then continue with the spec. This takes 30 seconds and dramatically improves spec quality.
