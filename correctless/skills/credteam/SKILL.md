...
This skill requires intensity `critical` or above. Read `.correctless/config/workflow-config.json` and check `workflow.intensity`. If the field is absent or not set, default to `standard`.
