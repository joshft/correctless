...
scan_artifacts() {
  ...
  local candidates=()
  local artifact_patterns="workflow-state-*.json token-log-*.jsonl qa-findings-*.json audit-trail-*.jsonl pipeline-manifest-*.json autonomous-decisions-*.jsonl escalation-*.json adherence-*.json antipattern-findings-*.json cost-cache-*.json review-decisions-*.json harness-notified-*.flag"
