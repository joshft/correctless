...
- **If found and <72 hours old**: Read `completed_phases` (e.g., `["round-1", "round-2"]`). Before skipping, verify each completed round: the findings artifact for that round must exist in `.correctless/artifacts/findings/` (e.g., `audit-{preset}-{date}-round-{N}.json`). If verification passes: "Found checkpoint from {timestamp} — rounds 1-{N} already done. Resuming from round {N+1}." Skip completed rounds. If a round's artifact is missing: restart from that round. On resume, also restore the in-progress round's SHA: if `round_start_sha` is present in the checkpoint, re-pin via `git update-ref "refs/audit-round-{N}-start" {round_start_sha}` before starting the round.
...
```json
{
  ...
  "round_start_sha": "{SHA pinned by Step 5.5 for current round}",
  "timestamp": "ISO"
}
```
