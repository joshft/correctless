...
- **Lock re-gated from `mkdir` to O_EXCL** (INV-007): mutual exclusion is now gated on the atomic `O_EXCL` (noclobber-redirect) creation of a `pid` file inside the retained `${dest}.lock` directory, because `mkdir` is not guaranteed atomic on every filesystem/emulation layer — non-atomic behavior was observed under some sandboxed test environments during this feature's concurrency testing. The empty-pid create→write window and dead-holder reclaim are handled with a grace re-check and atomic `mv`, and release removes the pid token then a best-effort `rmdir` (leaving a live dir a waiter has re-claimed).
