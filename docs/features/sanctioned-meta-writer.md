...
- **Lock re-gated to `flock` (with an ln/mkdir fallback)** (INV-007): mutual exclusion now uses kernel `flock` on a persistent `${sf}.flock` fd when available (the default on Linux/CI) — race-free and auto-released on process death — falling back to an atomic `ln` hard-link dir-lock where `flock` is absent (macOS; `CORRECTLESS_LOCK_IMPL=ln` forces it). The journey was mkdir → O_EXCL → ln → flock: `mkdir` was non-atomic on the sandbox overlay, the O_EXCL grace-loop raced on CI, and even the `ln` create-with-content still flaked once under the jq-1.8 runner's adversarial scheduling — file-creation locks with PID stale-detection can't guarantee exclusion under extreme scheduling, so only kernel flock is truly race-free. QA-002 stress-tests the flock path; the deterministic R-tests cover the ln fallback.
