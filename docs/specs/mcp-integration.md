...
- **R-002** [integration]: `/csetup` presents MCP as a single decision with four options: "both", "just Serena", "just Context7", "skip". The offer appears between Step 2 (setup script) and Step 3 (config review). The offer only appears when at least one server is not yet configured AND the required tooling (`uv` for Serena, `npx` for Context7) is available. For `project.language: "other"`, the Serena offer still appears with a caveat: "Serena works best with Python, TypeScript, Go, Rust, Java, PHP, Ruby, C#. Your project may have partial support."
...
- **Serena language support gaps** — some languages (C, C++, Haskell) aren't supported. Mitigation: `/csetup` only offers Serena for supported languages (Python, TypeScript/JavaScript, Go, Rust, Java, PHP, Ruby, C#). For `project.language: "other"`, the offer still appears with the partial-support caveat per R-002.
