...
# --- Lite skills: csetup cspec creview ctdd cverify cdocs crefactor cpr-review ccontribute cmaintain cstatus csummary cmetrics cdebug chelp cwtf cquick crelease cexplain ---
for skill in csetup cspec creview ctdd cverify cdocs crefactor cpr-review ccontribute cmaintain cstatus csummary cmetrics cdebug chelp cwtf cquick crelease cexplain; do
  if [ "$CHECK_ONLY" = false ]; then
    mkdir -p "correctless-lite/skills/$skill"
  fi
  sync_file "skills/$skill/SKILL.md" "correctless-lite/skills/$skill/SKILL.md"
done
[ "$CHECK_ONLY" = false ] && info "Lite skills (19) → correctless-lite"

# --- Full skills (all): csetup cspec cmodel creview creview-spec ctdd cverify caudit cupdate-arch cdocs cpostmortem cdevadv credteam crefactor cpr-review ccontribute cmaintain cstatus csummary cmetrics cdebug chelp cwtf cquick crelease cexplain ---
...
[ "$CHECK_ONLY" = false ] && info "Full skills (26) → correctless-full"
