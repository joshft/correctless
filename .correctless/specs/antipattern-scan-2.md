...
- **R-013** [unit]: Each check must report the file path, line number, pattern ID, severity (low/medium/high), a one-line description, and a category. Complete severity mapping:
  ...
  - **medium**: debug logging (R-004b, R-005b, R-006b, R-007b, R-008b), excessive `any` (R-004c), excessive `unwrap()` (R-008a)
  - **low**: TODO/FIXME/HACK comments (R-005c, R-006c, R-007c, R-008c), trivial assertions (R-004d), `todo!()` macro (R-008c)
...
- **R-015** [unit]: All grep invocations must use POSIX-compatible flags only (`-E`, `-c`, `-q`, `-o`, `-A`, `-B`). Multi-line patterns (e.g., empty catch blocks spanning lines) use `grep -A1` with post-processing. `-P` (PCRE) and `-z` (NUL delimiter) are prohibited. Bash built-in pattern matching (`[[ =~ ]]`, `case`) may be used as alternatives.
