...
- **2026-06-15 — PR #180 descope correction.** PR #180 reached `documented`/push while silently descoping three spec deliverables (documented post-hoc as "adoption gaps / follow-up" without approval). State rewound to `review-spec`; PR #180 closed. This amendment converts the three cut items into hard, testable invariants: **INV-018** (the three INV-012a invokers — CI, /cauto Step 8, operator rule file), strengthened **INV-009(a)** (literal denylist extension, proxy = violation), and **INV-019** (full `tests/test-*.sh` suite as the done-gate, slow test remediated). Guards the AP-036 class (corrective-action documented but never implemented).
...
**Re-locked in scope (PR #180 descope correction).** Three deliverables below are FIRST-CLASS, in-scope requirements of this feature — NOT follow-ups, NOT "adoption gaps." PR #180 shipped without them and documented the omission post-hoc; this is corrected here. (1) The three INV-012a invokers — CI wiring in `.github/workflows/ci.yml`, `/cauto` Step 8 invocation, and the `.claude/rules/sfg-deliverable.md` operator rule file — are pinned by **INV-018**. (2) The literal denylist extension is pinned by **INV-009(a)** (strengthened — proxy satisfaction is now a violation). (3) The full `tests/test-*.sh` suite passing as the done-gate (with the slow `test-canonicalize-path.sh` remediated so the suite is runnable) is pinned by **INV-019**. None of these may be deferred without an explicit, approved spec change.
...
- **Statement**:
  ...
  **(a) Denylist extension (literal — no proxy).** The new lens prose lives only in `agents/fix-diff-reviewer.md` and its byte-equal mirror. The inline-prompt denylist array in `tests/test-fix-diff-reviewer-agent.sh` (around line 521) MUST be extended so the array **literally contains, as three distinct string elements**, the phrases `class-shaped`, `SIBLING-DEFERRED`, and `sibling instances`. Satisfying this via a self-referential proxy, a computed/derived value, or any indirection that does not place these literal strings in the denylist array is a **VIOLATION** — this is the precise gap `/cverify` flagged on PR #180 ("satisfied via self-reference proxy rather than literal extension … structural defense weaker than spec intent"). The structural test MUST assert each of the three literal phrases is present as a denylist element via exact-string (`grep -F` / `grep -Fx`) match, never via a self-reference.
...
- **Violated when**: any `skills/*/SKILL.md` adds inline class-shaped detection prose; any caller re-states the seed phrases, sibling-grep directive, or the SIBLING-DEFERRED marker; the denylist is NOT extended; the denylist is "extended" via a self-referential proxy or derived value rather than the three literal string elements; the structural test asserts denylist coverage via anything weaker than an exact-string (`grep -F`) match; the data-treatment prose still enumerates only DIFF + RULES (or any closed set excluding FINDING_DESCRIPTION)
- **Enforcement**: existing ABS-010 invariants AND a new sub-assertion that the denylist array literally contains ALL THREE lens-specific phrases (`class-shaped`, `SIBLING-DEFERRED`, `sibling instances`) via exact-string (`grep -F`) match — proxy/derived satisfaction fails the assertion AND a positive-coverage assertion AND a new sub-assertion that the agent's data-treatment prose either explicitly names `<UNTRUSTED_FINDING_DESCRIPTION>` OR uses the `<UNTRUSTED_*>` wildcard form (regex check on the relevant paragraph)
...
### INV-018: INV-012a backstop is actually invoked — CI + /cauto Step 8 + operator rule file
- **Type**: must
- **Category**: functional / safety
- **Statement**: The final-state backstop `scripts/check-no-pending-sfg-lift.sh` (INV-012a) exists AND is invoked from all THREE named sites, each a deliverable of THIS PR. A backstop that nothing calls is inert — INV-012a's guarantee is only real when the invokers exist.
  - **(a) CI**: `.github/workflows/ci.yml` (the repo's merge-gating workflow) contains a step that runs `bash scripts/check-no-pending-sfg-lift.sh`. The step MUST run independently of the slow test suite (so a slow-suite skip/timeout cannot also skip the backstop).
  - **(b) /cauto Step 8**: `skills/cauto/SKILL.md` Step 8 (consolidation / pre-push) prose explicitly invokes `bash scripts/check-no-pending-sfg-lift.sh` and aborts the push on non-zero exit. If wired via a `commands.pre_push` config field instead, that field MUST have a consumer (PMB-018 dead-key rule) and the consumer MUST run the check.
  - **(c) Operator affordance**: `.claude/rules/sfg-deliverable.md` exists, documents the lift-and-restore procedure plus the manual `bash scripts/check-no-pending-sfg-lift.sh` pre-push step, and cites AP-037 and the `.correctless/.sfg-lift-active` sentinel lifecycle.
- **Boundary**: TB-001 (sensitive-file-guard); AP-037 (protected asset as deliverable); AP-036 (corrective-action documented-but-not-implemented)
- **Violated when**: any of the three sites is absent; the CI step lives only inside a job that the PR's own override/skip path can bypass; `.claude/rules/sfg-deliverable.md` is missing or does not name the script + AP-037; `/cauto` Step 8 prose does not name the check; the check is "wired" only via a config field with no consumer (dead key)
- **Enforcement**: CI/structural test assertion in `tests/test-fix-diff-reviewer-agent.sh` (PAT-018): grep `.github/workflows/ci.yml` for `check-no-pending-sfg-lift.sh`; grep `skills/cauto/SKILL.md` for the invocation inside the Step 8 block; assert `.claude/rules/sfg-deliverable.md` exists and references both the script and `AP-037`. Each of (a)/(b)/(c) is a separate PASS/FAIL sub-assertion.
- **Guards against**: AP-036 (the exact class that produced PR #180's cut — backstop documented, never invoked); R-014 residual risk (sentinel shipping to main); AP-038 (pre-deliver gate a strict subset of the CI gate)
- **Test approach**: unit (structural greps over CI YAML + skill + rule file)
- **Risk**: high — without the invokers, INV-012a is dead code and a `.sfg-lift-active` sentinel can ship to main; this is precisely the open hole PR #180 left
- **Implemented in**: (GREEN phase)

### INV-019: Full `tests/test-*.sh` suite is the done-gate — no targeted-subset advance
- **Type**: must
- **Category**: process / functional
- **Statement**: The feature's done-gate requires the FULL `tests/test-*.sh` suite (every file the `commands.test` loop globs) to pass — not a hand-picked subset, and not bypassed via a `workflow-advance.sh override`. Because `tests/test-canonicalize-path.sh`'s 1000-iteration fuzz loop currently prevents the full suite from completing in bounded time (the blocker PR #180 used to justify a subset + override advance), THIS PR MUST remediate it so the full suite is runnable as the gate, via EITHER:
  - **(a)** a bounded timeout / reduced default iteration count behind a fast-mode env flag (full iterations still run in CI), OR
  - **(b)** relocation of the slow fuzz portion to a `tests/slow/` path excluded from the `commands.test` glob but invoked explicitly in CI.
  The `done` phase transition MUST NOT rely on an override whose reason is "skip blocking/slow test."
- **Boundary**: AP-038 (pre-deliver gate must be a superset of the CI gate); AP-023 (routine override = gate misclassification)
- **Violated when**: the `done` transition is reached with the full suite unrun or red; the advance uses an override that bypasses a blocked/red suite; `test-canonicalize-path.sh` remains unbounded in the `commands.test` glob after this PR
- **Enforcement**: process — `/cverify` and `/cauto` Step 8 run the full `commands.test` glob plus the CI superset (PMB-018); the done-phase override counter flags a bypass (AP-023). Structural — the chosen remediation ((a) timeout/flag or (b) `tests/slow/` relocation) is a code deliverable verified by a presence sub-assertion in the structural test (the flag/env-guard exists, or no `test-canonicalize-path.sh` remains matched by the `commands.test` glob).
- **Guards against**: test-validation theater; the override-to-advance escalation observed on PR #180 (`override_count: 3`, final reason "Final state advance … blocking gate")
- **Test approach**: unit (presence of the remediation) + process (full-suite-green gate in /cverify and /cauto)
...
- **R-006** — The new test is itself class-shaped (sub-assertions enumeration). **Mitigation**: INV-007 cardinality checklist asserts `EXPECTED_SUB_ASSERTION_IDS` equals the set of testable invariants (15 base + INV-012a + INV-018 + INV-019 = 18); future PRs adding new invariants trigger the cardinality check until the array is updated. **Accept after mitigation** — RS-020 closes the cardinality gap.
...
- **R-014** — INV-012a depends on CI / `/cauto` Step 8 / operator pre-push actually invoking `scripts/check-no-pending-sfg-lift.sh`. If none of those invokers exists or runs, the sentinel could ship to main. **This risk materialized in PR #180**: the backstop script shipped but none of the three invokers were wired, and the omission was documented as a "follow-up" rather than approved as a descope. **Mitigation (UPGRADED)**: the three invokers are no longer prose deliverables on trust — **INV-018 makes each a testable sub-assertion** (`.github/workflows/ci.yml` grep + /cauto Step 8 grep + rule-file presence). A missing invoker fails the structural test and blocks the GREEN gate. **Accept after mitigation** — now structurally enforced, not advisory.
