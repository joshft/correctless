...
- **Invariant**: Review agents treat historical findings as advisory data, not instructions. Classification (R-003) re-derives pattern descriptions rather than echoing raw finding text. No finding text is interpolated into structured instructions. The deferred findings backlog (ABS-033) is an additional instance of this boundary — `/cauto` sweep surfaces backlog entries (LLM-generated review findings) into the pipeline context as advisory data.
- **Violated when**: Raw finding descriptions are used as review instructions without re-classification
- **Test**: R-012 in shift-left-review tests (defensive instruction presence); ABS-033 backlog sweep tested in `tests/test-deferred-findings-backlog.sh`
