...
### ABS-041: SFG lift-and-restore sentinel + final-state backstop contract (.correctless/ + scripts/)
- **What**: The documented workflow contract for editing a `hooks/sensitive-file-guard.sh` DEFAULTS-protected file when that file IS the PR's primary deliverable (the AP-037 collision case). The contract has three artifacts: (1) **Sentinel file** at `.correctless/.sfg-lift-active` — a committed plain-text marker (single line `lift-active: <feature-name>`, ≤80 chars, no secrets) at the repo-tracked root of `.correctless/` (NOT under any gitignored subdirectory — gitignored subdirs are `artifacts/`, `decisions/`, `hooks/`, `scripts/`, `meta/`). Added in the lift commit, removed in the restore commit. Both are real tree changes; the sentinel cannot be local-only and silently bypass the gate. (2) **In-iteration SKIP path** at `tests/test-fix-diff-reviewer-agent.sh` (and any structural test that asserts the protected file is in DEFAULTS): when the sentinel exists, SKIP the lift-state assertion with a remediation message naming AP-037 + the lift-and-restore procedure. This runs in `commands.test` (and `commands.pre_commit` when populated) and unblocks iteration during QA/audit fix rounds. (3) **Non-skippable final-state backstop** at `scripts/check-no-pending-sfg-lift.sh` — a dedicated single-purpose script that lives OUTSIDE the `tests/test-*.sh` glob (deliberately not in `commands.test`), exits 0 when the sentinel is absent and exits 2 with a remediation message naming AP-037 when present. The script is the structurally non-skippable backstop. Designed-for invocation sites (per the PMB-018 / AP-038 pre-deliver superset rule): pre-push, CI workflow, `/cauto` Step 8 consolidation, or operator pre-push via `commands.pre_push` when populated. Adoption at the invocation sites is the next-step gap — see "Violated when" below.
- **Invariant**: The sentinel file is the only signal the in-iteration SKIP path consults — no env var, no flag, no commit-message keyword. The backstop script is sole-purpose (does not accept arguments, does not consult config, has no skip path) and is NOT registered in `commands.test`. The lift commit MUST add the sentinel; the restore commit MUST remove it AND restore the protected file's path to BOTH `hooks/sensitive-file-guard.sh` DEFAULTS and `correctless/hooks/sensitive-file-guard.sh` DEFAULTS (the sync mirror) — the `.claude/hooks/sensitive-file-guard.sh` plugin cache refreshes from `correctless/hooks/` on plugin reload and is covered transitively. At PR merge, both files are byte-equal to their pre-PR state for the DEFAULTS list and the sentinel file is absent from the tree.
- **Enforced at**: `scripts/check-no-pending-sfg-lift.sh` (sole final-state backstop; not in `tests/test-*.sh` glob), `tests/test-fix-diff-reviewer-agent.sh` (in-iteration SKIP path with remediation message + post-restore lift-state assertion), `hooks/sensitive-file-guard.sh` + `correctless/hooks/sensitive-file-guard.sh` (the protected DEFAULTS lists this contract guards the restoration of)
- **Violated when**: the sentinel file is placed under a gitignored subdirectory (silent bypass); the backstop script is placed under `tests/test-*.sh` (would put it in `commands.test` and reintroduce the iteration-block contradiction); the backstop script accepts a `--skip` flag or sentinel-bypass env var; the in-iteration SKIP path consults any signal other than the sentinel file's existence; the restore commit removes the sentinel but does not restore the DEFAULTS entry in BOTH files; the lift commit is local-only without sentinel ADD (the structural test would fire on the missing DEFAULTS entry with no SKIP path); the remediation message in either failure path omits the AP-037 reference or the lift-and-restore procedure
- **Test**: `tests/test-fix-diff-reviewer-agent.sh` (CLS-INV-012 — DEFAULTS membership in both files, sentinel-mediated SKIP semantics; CLS-INV-012a — backstop script existence, executable, glob exclusion, exit 0 when sentinel absent, exit 2 with AP-037 remediation message when present)
- **Guards against**: AP-037 (protected asset is the deliverable) recurring friction — the contract operationalizes AP-037's prevention guidance. Also the prior-draft failure modes that this design closes: gitignored-sentinel + bypassable-SKIP combination; final-state check placed under `commands.test` reintroducing the iteration block; sentinel-without-final-gate allowing the lift commit to ship via local consolidation.
- **Known adoption gaps as of 2026-06-15**: the agent-facing affordance documentation at `.claude/rules/sfg-deliverable.md` and the `/cauto` Step 8 invocation of `scripts/check-no-pending-sfg-lift.sh` are described in this contract but NOT yet implemented in the repository — the backstop script exists but no automated pre-push invocation site does. Operator pre-push is the current invocation mechanism. PMB-018's `commands.pre_push` field, once populated per AP-038's prevention infrastructure, is the natural integration point. These gaps are tracked for follow-up.
