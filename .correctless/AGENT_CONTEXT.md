...
| Tests | `tests/test*.sh` | 51 test files (~4,090 shell tests) covering all hooks, scripts, skills, agents, Phase 2 components, Phase 3 components, test-evasion antipattern validation, scanner expansion (grep portability + dead-security-fn detection), auto-UX improvements (flexible phase entry, scoped consolidation, pipeline summary), scripts namespace migration, spec mutation alerts, and override frequency metrics (cross-run pattern detection + preservation + 50-file cap) |
